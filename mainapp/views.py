...
class BaseView(CartMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        categories_global = CategoryGlobal.objects.all()
...
class CategoryDetailView(CartMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        category_global = self.get_object()
        context['cart'] = self.cart
        context['categories'] = self.model.objects.all()
        context['category_global'] = self.model.objects.all()
        ...
        context['category_products'] = products
        context['category_global'] = products
        return context


class CategoryGlobalDetailView(CartMixin, DetailView):

    model = CategoryGlobal
    queryset = CategoryGlobal.objects.all()
    context_object_name = 'category_global'
    template_name = 'category_global_detail.html'
    slug_url_kwarg = 'slug'
    ...
    def get_context_data(self, **kwargs):
        ...
        category_global = self.get_object()
        context['cart'] = self.cart
        context['categories_global'] = self.model.objects.all()
        if not query and not self.request.GET:
            context['category_global_products'] = category_global.product_set.all()
            return context
        if query:
            products = category_global.product_set.filter(Q(title__icontains=query))
            context['category_global_products'] = products
            return context
        url_kwargs = {}
        for item in self.request.GET:
            if len(self.request.GET.getlist(item)) > 1:
